...
module.exports = {
  ...
  async getUser(req, res, next) {
    ...
    const user = await UserService.getUserByDocument(id);
  ...
  async putUser(req, res) {
    ...
    const userUpdate = await UserService.putUser(id, userReceived);
  ...
  async signUp(req, res) {
    ...
    user.save(async err => {
      ...
      const generatedToken = await UserService.createToken(user);
  ...
  async signIn(req, res) {
    User.findOne({ email: req.body.email }, (err, user) => {
      ...
      return user.comparePassword(req.body.password, async (err, isMatch) => {
        ...
        return res.status(200).send({ message: 'Te has logueado correctamente', token: generatedToken });
      });
