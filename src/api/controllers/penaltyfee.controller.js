...
module.exports={
    async getPenaltyfees(req, res, next) 
  {
    ...
    return res.status(200).send({ penaltyfees });
  ...
  async getPenaltyfee(req, res, next)
   {
    ...
    return res.status(200).send({ penaltyfee });
  },
  ...
  async postPenaltyfee(req,res,next)
  {
    ...
    return res.status(200).send({ newPenaltyfee });
  },
      ...
      {
        const {id}=req.params;
        let penaltyfeeReceived = req.body;
        penaltyfeeUpdate= await PenaltyfeeService.putPenaltyfee(id,penaltyfeeReceived);

        return res.status(200).send({penaltyfeeUpdate});
      },
